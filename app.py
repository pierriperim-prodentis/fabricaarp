...
if chave not in (VIEW_KEY, ADMIN_KEY):
    st.markdown(
        """
...
title_suffix = " · visão admin" if is_admin else ""
st.title(f"Painel de Vendas de Fábrica{title_suffix}")
...
chart_df = pd.DataFrame({"Mês": MONTH_SHORT, "Total": month_totals}).set_index("Mês")
...
totals = entry.get("totals_by_assessora", {})
rows = []
for a, v in sorted(totals.items(), key=lambda kv: -kv[1]):
    rows.append({"Assessora": title_case(a), "Loja": ASSESSOR_LOJA.get(a, "—"), "Total vendido": fmt_money(v)})
detail_df = pd.DataFrame(rows)
st.dataframe(detail_df, use_container_width=True, hide_index=True)
st.markdown(f"**Total do mês:** {fmt_money(sum(totals.values()))}")

# ------------------------------------------------------------------
# Admin-only: line-item detail
# ------------------------------------------------------------------
if is_admin:
    st.divider()
    st.subheader("🔑 Detalhamento linha a linha (admin)")
    details = entry.get("details", [])
    if details:
        det_df = pd.DataFrame(details)
        det_df = det_df.rename(columns={
            "assessora": "Assessora", "cliente": "Cliente", "data": "Data",
            "valor": "Valor", "instituicao": "Instituição",
        })
